...
public sealed class MainViewModel : INotifyPropertyChanged
{
    ...
    private JpegBackground jpegBackground = JpegBackground.White;
    public JpegBackground JpegBackground
    {
        get => jpegBackground;
        set
        {
            if (jpegBackground == value) return;
            jpegBackground = value;
            Raise(nameof(JpegBackground)); Raise(nameof(IsJpegBackgroundWhite)); Raise(nameof(IsJpegBackgroundBlack));
        }
    }
    public bool IsJpegBackgroundWhite { get => JpegBackground == JpegBackground.White; set { if (value) JpegBackground = JpegBackground.White; } }
    public bool IsJpegBackgroundBlack { get => JpegBackground == JpegBackground.Black; set { if (value) JpegBackground = JpegBackground.Black; } }
    ...
    public async Task ResizeFolderAsync()
    {
        ...
        JpegBackground selectedJpegBackground = JpegBackground;
            ...
            await Parallel.ForEachAsync(planned.Items.Select((item, index) => (item, index)),
                ...
                (entry, ct) =>
                {
                    try
                    {
                        ...
                        {
                            LargeImageBridge.Resize(entry.item.Path, destinationPath, targetWidth, targetHeight, format, selectedJpegBackground);
                        }
                        else
                        {
                            ...
                            ct.ThrowIfCancellationRequested();
                            ImageSharpBridge.Save(resized, destinationPath, format, selectedJpegBackground);
                        }
